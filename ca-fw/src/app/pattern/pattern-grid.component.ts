...
export class PatternGridComponent implements OnInit {
  ...
  /**
   * Creates tiles map which consist of an index of the tile as key and Tile object as value.
   */
   ...
   */
  private markAliveTiles(): void {
    this.aliveTiles = new Set<Tile>();
    this.pattern.coordinates.forEach((coordinate: PatternCoordinate) => {
      const coordinateNumber = this.getTileIndexFromRowAndColumn(coordinate.row, coordinate.column);
  ...
  nextGeneration(): void {
    ...
    this.aliveTiles.forEach((aliveTile: Tile) => {
      const neighborsOfTile = this.collectNeighborsOfTile(aliveTile);
      ...
      if (aliveNeighborCount < 2 || aliveNeighborCount > 3) {
        tilesDeadNextGeneration.add(aliveTile);
      }
    });
